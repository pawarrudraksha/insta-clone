...
export const createChat=createAsyncThunk("chat/create-chat",
  async(users:string[])=>{
    try {
      if(users?.length>1){
        const response=await axios.post("/chat/create",{users,isGroupChat:true,chatName:""})
...
export const getChatInfo=createAsyncThunk('chat/get-info',
  async(chatId:string)=>{
    try {
...
export const getUserChats=createAsyncThunk('chat/get-user-chats',
  async()=>{
    try {
...
export const deleteChat=createAsyncThunk(`chat/delete`,
  async(chatId:string)=>{
    try {
...
export const findChat=createAsyncThunk('chat/find',
...
export const getMessages=createAsyncThunk('chat/get-messages',
  async({chatId,page,limit}:{chatId:string,page:number,limit?:number})=>{
    try {
      if(limit){
        const response=await axios.get(`/messages/get-chat-messages/${chatId}?page=${page}&limit=${limit}`)
        return response.data
      }else{
        const response=await axios.get(`/messages/get-chat-messages/${chatId}?page=${page}`)
        return response.data
      }
    } catch (error) {
      console.log(error);
...
export const sendTextMessage=createAsyncThunk('chat/send-text-message',
  async({chatId,toReplyMessage,message}:SendTextType)=>{
    try {      
      if(toReplyMessage){
        const response=await axios.post('/messages/send-text',{chatId,message,toReplyMessage})        
        return response.data;
      }else{
        const response=await axios.post('/messages/send-text',{chatId,message})
        return response.data;
      }
    } catch (error) {
      console.log(error);
...
export const getMessageById=createAsyncThunk('chat/get-message-by-id',
  async ({chatId,messageId}:{chatId:string,messageId:string})=>{
    try {
      const response=await axios.get(`/messages/get-message?chatId=${chatId}&messageId=${messageId}`)
      return response.data
    } catch (error) {
      console.log(error);
    }
...
const initialState: MessagesState = {
    ...
    checkedUsers: [],
    currentChatInfo:{_id:"",updatedAt:'',users:[],admin:[],chatName:"",isGroupChat:false}
}
...
export const { toggleChatDetails,toggleNewMessageModal,toggleIsReplyMsg,setReplyMsg,addToCheckUsers,resetCheckedUsers,setCheckedUsers,setCurrentChatInfo} = messagesSlice.actions;
...
export const selectCheckedUsers = (state: RootState): User[] => state.messages.checkedUsers;
export const selectCurrentChatInfo = (state: RootState): ChatInfo=> state.messages.currentChatInfo;
