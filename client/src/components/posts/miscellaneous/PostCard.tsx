...
import {
  getPostByIdWhenLoggedIn,
  getPostByIdWhenNotLoggedIn,
  getPostCommentsWhenLoggedIn,
  getPostCommentsWhenNotLoggedIn,
  postComment,
...
const PostCard: React.FC = () => {
  ...
  const [post, setPost] = useState<Post>({
    _id: "",
    caption: "",
    userInfo: { _id: "", profilePic: "", username: "" },
    updatedAt: "",
    noOfLikes: -1,
    isPostLiked: false,
  });
    ...
    if (postId) {
      const fetchPosts = async () => {
        if (currentUser?._id) {
          ...
          const filteredPosts = await response?.payload?.data?.posts?.map(
            (item: ReceivedPostItem) => {
              return {
      ...
      const fetchComments = async () => {
        if (currentUser?._id) {
          const results = await dispatch(
            getPostCommentsWhenLoggedIn({ postId, page: commentsPage })
          );
          setPostComments(results?.payload?.data);
        } else {
          const results = await dispatch(
            getPostCommentsWhenNotLoggedIn({ postId, page: commentsPage })
          );
          setPostComments(results?.payload?.data);
        }
      };
  ...
  const handleBackspace = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (
      ...
      (!comment.trim() || inputRef.current?.selectionStart === 0)
    ) {
      dispatch(setToReplyComment({ username: "", commentId: "" }));
    }
  };
        ...
        <div className={styles.postCardContentContainer}>
          ...
          <div className={styles.postCardContent}>
            <div className={styles.postCardCaption}>
              ...
              <div className={styles.postCardCaptionContent}>
                <div className={styles.postCardCaptionInnerContent}>
                  <p>{post?.userInfo?.username}</p>
                  <p className={styles.postCardCaptionInnerContentCaption}>
                    {post?.caption}
                  </p>
                </div>
                <p className={styles.postCardCaptionContentDate}>
                  {postUpdatedTime}
                </p>
              </div>
            </div>
            <div className={styles.postCardCommentsContainer}>
              ...
              {postComments?.length < 1 && <p>This post has no comments yet</p>}
            </div>
          </div>
