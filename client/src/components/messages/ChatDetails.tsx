...
import { deleteChat, selectCurrentChatInfo } from '../../app/features/messagesSlice'
    ...
    <div className={styles.chatDetailsContainer}>
      ...
      <div className={styles.chatDetailsInfo}>
        ...
        <div className={styles.chatDetailsMembers}>
          {
            chatInfo && chatInfo?.users?.length>0 &&
            chatInfo?.users?.map((item,index)=>(
              <Link to={`/${item?.username}`} className={styles.chatDetailMemberWrapper} key={index}>
