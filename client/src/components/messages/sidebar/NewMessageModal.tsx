...
import { createChat, resetCheckedUsers, selectCheckedUsers, setCheckedUsers, toggleNewMessageModal } from '../../../app/features/messagesSlice'
...
const NewMessageModal:React.FC= () => {
  ...
  const handleSearchUsers=async(e:React.ChangeEvent<HTMLInputElement>)=>{
    ...
    if(searchQuery.trim()){
      const results=await dispatch(searchUsers({searchQuery:e.target.value,page:1,limit:6}))
    ...
    <div className={styles.newMessageOverlay} onClick={handleModalClose}>
      <div className={styles.newMessageModalContainer} onClick={handlePropagation}>
          ...
          <div className={styles.newMessageModalSearchBar}>
              ...
              <input type="text" placeholder='Search...' value={searchQuery} onChange={handleSearchUsers} />
          </div>
          ...
          <button className={styles.newMessageModalChatBtn} onClick={handleCreateChat}>Chat</button>
      </div>
    </div>
  )
}
