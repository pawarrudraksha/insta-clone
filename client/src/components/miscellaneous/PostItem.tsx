...
const PostItem: React.FC<{ item: AccountPost; showReelIcon?: boolean }> = ({
  item,
  showReelIcon,
}) => {
  const navigate = useNavigate();
  const [isInteractionVisible, setIsInteractionVisible] =
  ...
  return (
    <div
      className={`${styles.postItemContainer} ${
        isInteractionVisible && styles.postItemContainerInteractionActive
      }`}
    ...
    >
      ...
      )}
      {
        <div className={styles.postItemTypeIndicator}>
          {item?.post?.type === "reel" &&
            showReelIcon &&
            item?.isStandAlone && <BiSolidMoviePlay />}
