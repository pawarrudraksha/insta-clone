  ...
  const handleChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (e.target.value !== "") {
      const data = await dispatch(
        searchUsers({ searchQuery: e.target.value, page, limit })
      );
  ...
  return (
    ...
    >
      ...
      <div className={styles.searchHeader}>
        <div
          className={styles.searchHeaderInputContainer}
      ...
      <div className={styles.searchContent}>
        {!searchQuery && (
          <div className={styles.searchContentHeader}>
            <p>Recent</p>
            <button className={styles.searchContainerClearAll}>
              Clear All
            </button>
          </div>
        )}
              ...
              >
                ...
                <div className={styles.searchResultContent}>
                  <div className={styles.searchResultInfo}>
                    <p className={styles.searchResultInfoUserName}>
                      {user?.username}
                    </p>
                    <div className={styles.searchResultInfoNameAndFollowers}>
                      ...
                      <BsDot />
                      <p>
                        {user?.noOfFollowers.length > 1
                          ? `${user?.noOfFollowers} followers`
                          : `${user?.noOfFollowers} follower`}
                      </p>
                    </div>
                  </div>
