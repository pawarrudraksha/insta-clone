...
const createHighlight = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (isStoryValidResults.some((valid) => !valid)) {
      throw new ApiError(
        401,
        "One or more story IDs are not valid or do not belong to the logged-in user"
      );
    }
...
const addStoryToHighlight = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
...
const updateHighlight = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (
      updatedHighlight.caption !== caption &&
      updatedHighlight.coverPic !== coverPic
    ) {
...
const removeStoryFromHighlight = asyncHandler(
  async (req: AuthenticatedRequest, res) => {
    ...
    if (highlight.stories.length === 1) {
      ...
      res
        .status(200)
        .json(new ApiResponse(200, {}, "highlight deleted successfully"));
    } else {
      const updatedHighlight = await Highlight.findByIdAndUpdate(
        highlightId,
        {
...
const deleteHighlight = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    res
      .status(200)
      .json(new ApiResponse(200, {}, "Highlight removed successfully"));
  }
);

const getHighlightById = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    const { highlightId } = req.params;
    if (!highlightId) {
    ...
    const highlightUser = await User.findById(highlight.userId).select({
      _id: 1,
      username: 1,
    });
    const isUserFollowRequestUser = await Follow.findOne({
      userId: highlightUser._id,
      follower: user._id,
...
const getUserHighlightsCoverPics = asyncHandler(
  async (req: Request | AuthenticatedRequest, res: Response) => {
    ...
    if (
      ...
      !isFollow &&
      currentUser &&
      currentUser._id.toString() !== requestedUser._id.toString()
    ) {
    ...
    if (highlights.length < 1) {
      return res
        .status(204)
        .json(new ApiResponse(204, {}, "No highlights for user"));
    }
    res
      .status(200)
      .json(
