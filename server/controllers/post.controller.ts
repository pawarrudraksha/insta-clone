...
const editPostCaption = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
...
const getPostsByUsername = asyncHandler(
  async (req: AuthenticatedRequest | Request, res: Response) => {
    ...
    if (posts.length < 1) {
      res.status(204).json(new ApiResponse(204, {}, "No posts"));
      return;
    }
    res
      .status(200)
...
const removePostItemFromPosts = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (isPost.userId.toString() !== user._id.toString()) {
      throw new ApiError(
        401,
        "Invalid request : only user can modify his posts"
      );
    }
    const postItemExists = isPost.posts.some(
      (item) => item._id.toString() === postItemId
    );
    if (!postItemExists) {
...
const addPostItemToPosts = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    res
      .status(200)
      .json(new ApiResponse(200, {}, "Post item added successfully to posts"));
  }
);

const tagUser = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    res.status(200).json(new ApiResponse(200, {}, "user tagged successfully"));
  }
);

const untagUser = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    res
      .status(200)
      .json(new ApiResponse(200, {}, "user untagged successfully"));
  }
);

const getAllPublicPosts = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    const page = Number(req.query.page || 1);
    ...
    if (!posts || posts.length < 1) {
      return res
        .status(204)
        .json(new ApiResponse(204, posts, "No public data available"));
    }
    res
      .status(200)
      .json(new ApiResponse(200, posts, "Posts data fetched successfully"));
  }
);

const getTaggedPosts = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    const page = Number(req.query.page || 1);
    const limit = Number(req.query.limit || 9);
    const skip = (page - 1) * limit;
    ...
    if (
      ...
      !isFollow &&
      requestedUser._id.toString() !== user._id.toString()
    ) {
    ...
    res
      .status(200)
      .json(new ApiResponse(200, posts, "tagged posts fetched successfully"));
  }
);

const getUserFeed = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
...
const getUserReels = asyncHandler(
  async (req: Request | AuthenticatedRequest, res: Response) => {
    ...
    if (reels.length < 1) {
      return res
        .status(204)
        .json(new ApiResponse(204, {}, "No data available"));
    }
    res
      .status(200)
      .json(new ApiResponse(200, reels, "Reels fetched successfully"));
  }
);

const getAllPublicReels = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    const page = Number(req.query.page || 1);
    const limit = Number(req.query.limit || 4);
    const skip = (page - 1) * limit;
    ...
    const posts = await Post.aggregate([
      ...
      },
      {
        $addFields: {
          isReelLiked: {
            $cond: {
              if: { $in: [currentUser?._id, "$likes.userId"] },
              then: true,
              else: false,
            },
          },
    ...
    res
      .status(200)
      .json(new ApiResponse(200, posts, "Reels fetched successfully"));
  }
);
