...
const likeAction = asyncHandler(
  async (req: LikeActionRequest, res: Response) => {
    ...
    if (targetType === "post") {
      ...
      alreadyLiked = await Like.findOne({
        commentId: targetId,
        userId: user._id,
      });
...
const getAllLikes = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (likes.length < 1) {
      return res
        .status(204)
        .json(new ApiResponse(204, {}, "no likes to fetch"));
    }
    res
      .status(200)
