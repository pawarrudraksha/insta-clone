...
const commentOnPost = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    res
      .status(201)
      .json(new ApiResponse(201, { isComment }, "Comment on post successful"));
  }
);
...
const getPostComments = asyncHandler(
  async (req: Request | AuthenticatedRequest, res: Response) => {
    ...
    if (
      postOwner.isPrivate &&
      !isFollow &&
      postOwner._id.toString() !== user._id.toString()
    ) {
    ...
    if (comments.length < 1) {
      return res
        .status(204)
        .json(new ApiResponse(204, {}, "No comments on post"));
    }
    res
      .status(200)
...
const getRepliesToComment = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    const { commentId } = req.params;
    ...
    res
      ...
      .json(
        new ApiResponse(
          200,
          replies,
          "Replies to the comment fetched successfully"
        )
      );
  }
);
