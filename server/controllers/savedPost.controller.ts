...
const savePost=asyncHandler(async(req:AuthenticatedRequest,res:Response)=>{
    ...
    const isFollow=await Follow.findOne({userId:postOwner._id,follower:user._id,isRequestAccepted:true})
    if(postOwner.isPrivate && !isFollow){
        throw new ApiError(401,"access denied")
    }
    const isPostAlreadySaved=await SavedPost.findOne({savedPostId:postId,userId:user._id})
    ...
    res.status(201).json(
        new ApiResponse(201,isPostSaved,"Saved post")
    )
})
...
const unsavePost=asyncHandler(async(req:AuthenticatedRequest,res:Response)=>{
    ...
    const isPostSaved=await SavedPost.findOne({savedPostId:postId,userId:user._id})    
    if(!isPostSaved){
...
const getAllSavedPosts = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const user = req.user;
    ...
    const savedPosts = await SavedPost.aggregate([
        ...
        if (savedPosts.length < 1) {
            return res.status(204).json(
                new ApiResponse(204, {}, "User does not have any saved posts")
            );
        }
