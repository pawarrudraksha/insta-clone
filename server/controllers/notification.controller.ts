...
const createNotification = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (receiverId === user._id.toString()) {
      throw new ApiError(
        400,
        "Receiver id and logged in user id cannot be same"
      );
    }
    if (postId) {
    ...
    res
      .status(201)
      .json(new ApiResponse(201, notification, "Notification sent"));
  }
);

const getReceivedNotifications = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (notifications.length < 1) {
      return res
        .status(200)
        .json(new ApiResponse(200, {}, "User has no received notifications"));
    } else {
      res
        .status(200)
        .json(
          new ApiResponse(
            200,
            notifications,
            "Received notifications fetched successfully"
          )
        );
    }
  }
...
const deleteNotificationById = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (
      !(
        notification.senderId.toString() === user._id.toString() ||
        notification.receiverId.toString() === user._id.toString()
      )
    ) {
...
const deleteNotificationByPostId = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
