...
const createChat = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (isGroupChat) {
      data.admin.push(user._id), (data.chatName = chatName);
    }
...
const addUserToChat = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    res.status(200).json(new ApiResponse(200, {}, "user added to group"));
  }
);

const removeUserFromChat = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
...
const exitGroup = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    res
      .status(200)
      .json(new ApiResponse(200, {}, "User exited group successfully"));
  }
);
...
const getChatInfo = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    const { chatId } = req.params;
    if (!chatId) {
      throw new ApiError(400, "Chat id is required");
    ...
    const chats = await Chat.aggregate([
      ...
      },
      {
        $project: {
          userInfo: {
            $filter: {
              input: "$chatInfo.userInfo",
              cond: { $ne: ["$$this._id", user?._id] },
            },
          },
