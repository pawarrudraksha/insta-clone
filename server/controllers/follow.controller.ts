...
const followUser=asyncHandler(async(req:AuthenticatedRequest,res:Response)=>{
    ...
    const isFollowAlreadyExist=await Follow.findOne({userId:requestedUserId,follower:currentUser._id})
    ...
    res.status(200).json(
        new ApiResponse(200,{},`${currentUser.username} has sent follow request to ${isUser.username}`)
    )
})
...
const getFollowRequests=asyncHandler(async(req:AuthenticatedRequest,res:Response)=>{
    ...
    if(followRequests.length<1){
        return res.status(204).json(
            new ApiResponse(204,{},"No follow requests for user")
        )
    }
    res.status(200).json(
        new ApiResponse(200,followRequests,"Follow requests fetched successfully")
    )
})
...
const getFollowers=asyncHandler(async(req:AuthenticatedRequest,res:Response)=>{
    ...
    if (requestedUser.isPrivate && requestedUser._id.toString() !== currentUser._id.toString()) {
        ...
        },
        {
            $addFields: {
                "followerDetails.hasStory": {
                    $cond: { if: { $gt: [{ $size: "$hasStory" }, 0] }, then: true, else: false }
                }
            }
        },
        {
    ...
    if(followers.length<1){
        res.status(204).json(
            new ApiResponse(204,{},"No followers for user")
        )
    }
    res.status(200).json(
...
const getFollowing=asyncHandler(async(req:AuthenticatedRequest,res:Response)=>{
    ...
    if(following.length<1){
        res.status(204).json(
            new ApiResponse(204,{},"No following for user")
        )
    }
    res.status(200).json(
        new ApiResponse(200,following,"Followers fetched successfully")
)})

const acceptFollowRequest=asyncHandler(async(req:AuthenticatedRequest,res:Response)=>{
    ...
    res.status(200).json(
        new ApiResponse(200,{},"Follow request accepted")
    )
})

const deleteFollowRequest=asyncHandler(async(req:AuthenticatedRequest,res:Response)=>{
    const {followDocId}=req.params
    if(!followDocId){
    ...
    if (followDoc.userId.toString() !== user._id.toString() && followDoc.follower.toString() !== user._id.toString()) {
    ...
    res.status(200).json(
        new ApiResponse(200,{},"Follow request deleted")
    )
})
...
const unFollow=asyncHandler(async(req:AuthenticatedRequest,res:Response)=>{
    ...
    const followDoc=await Follow.findOne({follower:user._id,userId:requestedUserId,isRequestAccepted:true})
    if(!followDoc){
...
const removeFollower=asyncHandler(async(req:AuthenticatedRequest,res:Response)=>{
    ...
    const followDoc=await Follow.findOne({follower:requestedUserId,userId:user._id})
    if(!followDoc){
        throw new ApiError(404,"Invalid request as you dont follow user")
    }
    await Follow.findByIdAndDelete(followDoc._id)
    ...
    res.status(200).json(
        new ApiResponse(200,{},"Removed follower successfully")
    )
})
